...
async function queryActiveTab() {
  // A JavaScript Promise is exactly that buzzer. It's an object standing in for a result that will arrive later, and it's always in one of those three states.
...
// Start a listener that watches every tab update. When your tab reports "complete", it resolves the Promise.
// Start a timeout that rejects after 20s so you don't wait forever.
// Whichever fires first "wins" — and it cleans up by removing the listener and clearing the timeout so nothing leaks.
...
async function scrapeTemporaryTab<T>(url: string, message: unknown) {
  ...
  try {
    ...
    // you'd leak invisible background tabs on every failure.
...
//async/await + loops for sequential scraping
...
function canGenerateExplanation(settings: AISettings) {
  if (settings.provider === "none") return false;
  if (settings.provider === "gemini") return Boolean((settings.geminiApiKey || settings.apiKey).trim());
  if (settings.provider === "openai") return Boolean((settings.openaiApiKey || settings.apiKey).trim());
  if (settings.provider === "claude") return Boolean((settings.claudeApiKey || settings.apiKey).trim());
  if (settings.provider === "deepseek") return Boolean((settings.deepseekApiKey || settings.apiKey).trim());
  if (settings.provider === "kimi") return Boolean((settings.kimiApiKey || settings.apiKey).trim());
...
async function analyzeCurrentTab(settings: AISettings): Promise<AnalysisResult> {
  ...
  if (shouldExplain) {
    try {
      explanation = await explainDiff(failedSubmission, problemStatement, matchedSubmissions, settings);
    } catch (error) {
      console.warn(error);
