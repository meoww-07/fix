...
export function extractLogicFingerprint(code: string): LogicFingerprint {
  ...
  if (
    /while\s*\(\s*(l|left)\s*[<!=]=?\s*(r|right)|while\s*\(\s*(r|right)\s*[<!=]=?\s*(l|left)/i.test(code) ||
    /\b(l|left)\s*\+\+[\s\S]{0,180}\b(r|right)\s*--|\b(r|right)\s*--[\s\S]{0,180}\b(l|left)\s*\+\+/i.test(code)
  ) {
  ...
  if (/\b(vector|array|vi|vll|vvi|vl|vc)\b|\[[^\]]+\]/.test(lower)) dataStructures.push("array/vector");
  ...
  if (/\bstring\b|\bchar\s+\w+\s*\[/.test(lower)) dataStructures.push("string/char buffer");
  if (/\bbitset\b/.test(lower)) dataStructures.push("bitset");
